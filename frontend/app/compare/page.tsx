...
export default function ComparePage() {
  ...
  const [contentA, setContentA] = useState<ContentItem>({
    id: "A",
    type: "text",
  });
  const [contentB, setContentB] = useState<ContentItem>({
    id: "B",
    type: "text",
  });
  ...
  const canAnalyze =
    mode === "text"
      ? contentA.text &&
        contentA.text.length >= 10 &&
        contentB.text &&
        contentB.text.length >= 10
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const difference = Math.abs(
        resultA.aiProbability - resultB.aiProbability
      );
  ...
  const analyzeContent = async (
    ...
    if (mode === "text" && content.text) {
      ...
      return {
        ...
        classification: getClassification(result.aiProbability),
        confidence: result.confidence,
        model: result.detectionModel,
        contentHash: "generated",
      };
    } else if (mode === "image" && content.image) {
      const result = await detectImage(content.image, "both");
      const aiProb =
        result.overall?.ai_probability ?? result.ai_probability ?? 50;
      return {
        aiProbability: aiProb,
        classification: getClassification(aiProb),
        confidence: result.confidence ?? 80,
        model: result.detection_model ?? "ensemble",
        contentHash: result.content_hash ?? "generated",
      };
    }
            ...
            <Card className="bg-gradient-to-r from-solana-purple/5 to-solana-green/5">
              <CardContent className="text-center">
                      ...
                      <span
                        className={cn(
                          ...
                          getClassificationColor(
                            comparison.A?.classification || "mixed"
                          )
                        )}
                      >
                    ...
                    <div className="flex-1 text-left">
                      <span
                        className={cn(
                          "text-2xl font-bold",
                          getClassificationColor(
                            comparison.B?.classification || "mixed"
                          )
                        )}
                      >
                ...
                <div className="mt-6 p-4 rounded-xl bg-white/50 dark:bg-gray-800/50 text-sm text-gray-600 max-w-lg mx-auto">
                  {comparison.winner === "tie" ? (
                    <p>
                      Both contents have similar AI detection scores (within
                      5%). The difference is not significant enough to determine
                      which is more AI-generated.
                    </p>
                  ) : comparison.winner === "A" ? (
                    <p>
                      <strong>Content A</strong> shows stronger indicators of AI
                      generation with a {comparison.difference.toFixed(1)}%
                      higher probability score than Content B.
                    </p>
                  ) : (
                    <p>
                      <strong>Content B</strong> shows stronger indicators of AI
                      generation with a {comparison.difference.toFixed(1)}%
                      higher probability score than Content A.
                    </p>
                  )}
                </div>
              </CardContent>
        ...
        <Card className="border-dashed">
          <CardContent className="text-center py-12">
            ...
            <p className="text-sm text-gray-500 max-w-md mx-auto">
              Add content to both sides above, then click "Compare & Analyze" to
              see which one is more likely to be AI-generated.
            </p>
          </CardContent>
        </Card>
      )}
